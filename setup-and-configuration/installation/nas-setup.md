...
SECTION 1 - SERVER SETUP – MAIN MARKET
...
The Copyinv & Copychq options can be done within NAS using the new util to
move these files. From a sale menu enter 997 and press enter.

[pic]

Use the Find Copy History Text Files option to do this for you.
    ...
    • VBINPUT is not needed if you have SALEMENU1 Y in SYSCONDAT
...
You will need to create a userdetails and useraccesstypes table before
opening NAS but if you have the new NLDBDEF.txt file from
QB\MARPROGS\MYSQLDEF folder and put this in the MARKET folder the tables
should be built automatically for you.
...
SECTION 2 - REMOTE SERVER OR LAPTOP  SERVER
...
[pic]
 ...
 • Create a NEW USER for this laptop server such as FARM, SHAFTS etc
   different to the main username which is used for the market. This is so
   customers update correctly from the main server. Add this to the
   userdetails and remoteservers table making sure your RSID’s match.


 • Make sure ‘MARKNAME’ is in the MASTER folder on the laptop server. (if
   not copy from MARKET on main server.) Login using your new NAS icon for
   this farm/remote sale (so working from farmsale laptop). Go to option 9,
   then 3 – amend office details, remove the sale names you will not be
   using, don’t change the order of the sale names, must be same as on main
   market NAS.
 ...
 • For the other machines on the remote network
      ...
      o Copy ROOTOFF and ROOTSHAFT (in this example) to the client machine
        (c:\) .
      o Use the same shortcut to START.BAT to run NAS

Test the customer and sales ledger update from the Main NAS menu – option
15
Make a change to a customer on the server then run the update and see if
the customer has also changed. Then vice versa.
...
SECTION 3 - Working at Remote Farmsale (no internet – eg in a field)

    • This is pretty much the same as section 2 but debtdir is different so
      please also read through section two first as covers in more detail..
    • Edit MYSQL database on laptop sysinfo – Mainserver – N
    • Create a new user – userdetails and remoteserver tables
    ...
    • Inside nldata create the followings folders
         ...
         o Copy across the MASTER, NLPROGS and the relevant sales folders
               ...
               • Edit MARKNAME – removing sale names not needed. Can do this
                 is NAS later on as explained in section2.
