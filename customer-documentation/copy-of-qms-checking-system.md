---
title: "Copy Of Qms Checking System"
source: "Copy of QMS Checking System.doc"
...
The Download Folder defaults to C:\QMS\ this will be fine.
[pic]

If you have Broadband/ADSL or ISDN then the  connection  to  select  is  LAN
otherwise select the dial-up connection name  you  use  to  connect  to  the
Internet.
[pic]

The majority of users programs are in C:\PROGRAMS\
[pic]

Answer Yes if you are a Newline using Market.
[pic]
...
If you pre-enter the lots as normal when the eartag and DOB are entered the
QMS checking begins. If the Vendor is farm assured and the eartag number
starts with UK5 then (born in Scotland) if the date of birth is before 01-
01-2004 the farm assured status defaults to Q.  If the date of birth is
after 31-12-2003 then the beast might be QMS approved but the system will
run the QMS checker.  If the eartag number starts with anything other than
UK5 (not born in Scotland) farm assured status default to Y. If none of
this applied the farm assured status=N.

 The QMS Checker Screen.
...
Salering & Batching Changes
 
If farm assured status = 'Q' then display SA on board and on batching
screen.
If farm assured status = 'Y' then display FA on board and on batching
screen.
If farm assured status= 'N'  then display NA on board and on batching
screen.
